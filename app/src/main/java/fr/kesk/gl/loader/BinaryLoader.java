...
import fr.kesk.gl.GlAssets.Geometry;
import fr.kesk.gl.GlAssets.Light;
import fr.kesk.gl.GlAssets.Material;
import fr.kesk.gl.GlAssets.Node;
import fr.kesk.gl.GlAssets.Node.GeometryInstance;
import fr.kesk.gl.GlAssets.Node.GeometryInstance.MaterialInstance;
import fr.kesk.gl.GlAssets.Scene;
import fr.kesk.gl.tools.ByteBufferPool;
import fr.kesk.gl.tools.MatrixUtils;
import fr.kesk.gl.tools.NodeUtils;
import fr.kesk.gl.buffer.GlBuffer;
import fr.kesk.gl.texture.ETC1GlTexture;
			...
			while(dataIn.available() > 0){
				...
				switch(entryType){
					case API.GEOMETRY:
						...
						final Geometry geometry = new Geometry(gId, new GlAssets.Geometry.Element[pCount]);
							...
							if(MODE == ASYNC){
								...
								contextManager.runOnGLThread(new Runnable() {
									...
									public void run() {
										...
										geometry.elements[elementId] = new GlAssets.Geometry.Element(pType, vCount,bufferSize, pMaterialId, inputs, handles[0], null);
								...
								geometry.elements[pIndex] = new GlAssets.Geometry.Element(pType, vCount, bufferSize, pMaterialId, inputs, GlBuffer.UNBIND_HANDLE, values); 
							}
						}
					...
					case API.SCENE:
						...
						for(int nIndex=0; nIndex< sNodes.length; nIndex++){
							sNodes[nIndex] = NodeUtils.newInstance(assets.nodes.get(dataIn.readInt()));
