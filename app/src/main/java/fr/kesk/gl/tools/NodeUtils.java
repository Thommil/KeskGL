...
public class NodeUtils {
	...
	/**
	 * Build a new instance of a node and subnodes and prepare it for Matrix computation
	 * 
	 * @param sourceNode The node to instanciate
	 * 
	 * @return A Node copy instance to be used in scene
	 */
	public static Node newInstance(Node sourceNode){
		final Node instance = new Node(sourceNode.id, sourceNode.type);
		System.arraycopy(sourceNode.model, 0, instance.model, 0, 16);
		if(sourceNode.cameraInstances != null){
			instance.cameraInstances = sourceNode.cameraInstances.clone();
		}
		if(sourceNode.lightInstances != null){
			instance.lightInstances = sourceNode.lightInstances.clone();
		}
		if(sourceNode.nodeInstances != null){
			instance.nodeInstances = new Node[sourceNode.nodeInstances.length];
			for(int nodeIndex=0; nodeIndex < sourceNode.nodeInstances.length; nodeIndex++){
				instance.nodeInstances[nodeIndex] = newInstance(sourceNode.nodeInstances[nodeIndex]);
			}
		}
		if(sourceNode.geometryInstances != null){
			instance.geometryInstances = sourceNode.geometryInstances.clone();
		}
