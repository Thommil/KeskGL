 ...
 * KESKGL->
 * Once again modified :
 * 	- user inputs using ContextManager->runOnGLThread()
 */
#include <jni.h>
...
#define LOGI(...) ((void)__android_log_print(ANDROID_LOG_INFO, "kesk_native_app_glue", __VA_ARGS__))
#define LOGV(...) ((void)__android_log_print(ANDROID_LOG_VERBOSE, "kesk_native_app_glue", __VA_ARGS__))
...
void** android_app_read_user_data(int fd, struct android_app* android_app) {
    void** data;
    if (read(fd, &data, android_app->userCmdDataSize) == android_app->userCmdDataSize) {
    	return data;
    }
...
static void android_app_destroy(struct android_app* android_app) {
    ...
    if(android_app->onDestroy){
		android_app->onDestroy(android_app);
	}
...
static void process_input(struct android_app* app, struct android_poll_source* source) {
    ...
    if (AInputQueue_getEvent(app->inputQueue, &event) >= 0) {
		// This is too high a rate for touch events.  Enable for debug if desired
//        LOGV("New input event: type=%d\n", AInputEvent_getType(event));
        ...
        LOGV("Failure reading next input event: %s\n", strerror(errno));
    }
}
...
static int uiLooperCallback(int fd, int events, void* data){
	struct android_poll_source *source = (struct android_poll_source*)data;
	if (source != NULL && source->app != NULL){
		void** userData = android_app_read_user_data(fd,source->app);
		if (userData != NULL) source->app->onUserCmd(source->app, userData);
	}
	return 1;
}

static void process_user_cmd(struct android_app* app, struct android_poll_source* source) {
    void** data = android_app_read_user_data(app->glmsgread,app);
	if (data != NULL) app->onUserCmd(app, data);
}
...
static void* android_app_entry(void* param) {
    ...
    android_app->userCmdPollSource.id = LOOPER_ID_USER;
	android_app->userCmdPollSource.app = android_app;
	android_app->userCmdPollSource.process = process_user_cmd;

    ALooper* looper = ALooper_prepare(ALOOPER_PREPARE_ALLOW_NON_CALLBACKS);
    ALooper_addFd(looper, android_app->msgread, LOOPER_ID_MAIN, ALOOPER_EVENT_INPUT, NULL,
            &android_app->cmdPollSource);
    ALooper_addFd(looper, android_app->glmsgread, LOOPER_ID_USER, ALOOPER_EVENT_INPUT, NULL,
                &android_app->userCmdPollSource);
...
static struct android_app* android_app_create(ANativeActivity* activity,
    ...
    int glmsgpipe[2];
	if (pipe(glmsgpipe)) {
		LOGI("could not create GL pipe: %s", strerror(errno));
	}
	android_app->glmsgread = glmsgpipe[0];
	android_app->glmsgwrite = glmsgpipe[1];

    int uimsgpipe[2];
	if (pipe(uimsgpipe)) {
		LOGI("could not create UI pipe: %s", strerror(errno));
	}
	android_app->uimsgread = uimsgpipe[0];
	android_app->uimsgwrite = uimsgpipe[1];

    android_app->uithread = pthread_self();
	...
	ALooper_addFd(looper, android_app->uimsgread, LOOPER_ID_USER, ALOOPER_EVENT_INPUT, &uiLooperCallback, &android_app->userCmdPollSource);
	android_app->uiLooper = looper;
...
static void android_app_free(struct android_app* android_app) {
    ...
    ALooper_removeFd(android_app->uiLooper, android_app->uimsgread);
    ...
    close(android_app->glmsgread);
    close(android_app->glmsgwrite);
    close(android_app->uimsgread);
    close(android_app->uimsgwrite);
...
void kesk_app_write_gl_user_cmd(struct android_app* android_app, void* data) {
    if (write(android_app->glmsgwrite, &data, android_app->userCmdDataSize) != android_app->userCmdDataSize) {
        LOGI("Failure writing android_app GL user cmd: %s\n", strerror(errno));
    }
}

void kesk_app_write_ui_user_cmd(struct android_app* android_app, void* data) {
    if (write(android_app->uimsgwrite, &data, android_app->userCmdDataSize) != android_app->userCmdDataSize) {
        LOGI("Failure writing android_app UI user cmd: %s\n", strerror(errno));
    }
}
